...
public class Left_Middle2Cycle extends SequentialCommandGroup {
  ...
  ) 
  {
    ...
    try {
      addCommands(
        AutoBuilder.resetOdom(path.LeftTrench_Center6.getStartingHolonomicPose().get()),
        Commands.parallel(
          AutoBuilder.followPath(path.emptyLeftTrench6),
          shooter.runRPMCommand(3000).withTimeout(0.05)
        ),
        Commands.race(
          AutoBuilder.followPath(path.LeftTrench_Center6), //go to center
          Commands.sequence(
        ...
        AutoBuilder.followPath(path.LeftPickUp_LeftTrench6), //go to shoot position
        Commands.race( //shoot
        ...
        Commands.parallel(
          Commands.race(
            AutoBuilder.followPath(path.LeftTrench_Center26),
        ...
        AutoBuilder.followPath(path.LeftCenter_Pickup26).raceWith(intake.runVolts(10.8)), //pickup and intake again
        AutoBuilder.followPath(path.LeftPickUp_LeftTrench26), //go to shoot position again
